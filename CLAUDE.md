...
- **Vite 8 / Vitest 5**: Client uses Vite 8 (upgraded from Vite 7). Vite 8 uses Rolldown instead of Rollup as its production bundler. Use Vitest 5 across all packages (upgraded from Vitest 4). `@vitejs/plugin-react` v6 dropped built-in Babel support (Vite 8 handles React Refresh via Oxc natively) — this project doesn't use Babel options so no impact. Vite 8 requires Node `>=20.19.0 || >=22.12.0`; Vitest 5 requires Node `>=22` and Vite `>=6.4`, both already satisfied (`engines.node` is `>=24.0.0` and the client is on Vite 8). Three Vitest 5 changes to keep in mind when writing tests:
  - **`clearMocks` now defaults to `true`**, i.e. `vi.clearAllMocks()` runs before every test. Call history therefore does _not_ accumulate across `it` blocks in a file; implementations survive (that would be `resetMocks`). Specs that need a stronger reset still call `mockReset()` themselves — see `claude-service.test.ts` and `allowed-emails.test.ts`.
  - **An async assertion that isn't awaited now fails the test**, and a hoisted `vi.mock` outside the top-level scope now throws instead of silently misbehaving. Both are strictness increases that turn previously-silent bugs into failures.
  - **Config files are no longer looked up from ancestor directories.** Every package must have its own config in its own directory — which the per-package `vitest.config.ts` files already do (the client falls back to `apps/client/vite.config.ts`). `configDefaults` survived the major, so the `dist` exclusion described above is unchanged.
- **pnpm.overrides**: **Currently empty — no active overrides.** The last one, `"ws@>=8.0.0 <8.21.0": "8.21.0"` (added 2026-06-15 for GHSA / Dependabot #31, a memory-exhaustion DoS in `ws < 8.21.0`), was removed on 2026-08-26 once its documented remove-condition was met: `engine.io@6.6.9`, `engine.io-client@6.6.6`, and `socket.io-adapter@2.5.8` all raised their declared `ws` range from `~8.20.1` to `~8.21.0`, and each is inside the range its parent already declares (`socket.io` wants `engine.io: ~6.6.0` and `socket.io-adapter: ~2.5.2`; `socket.io-client` wants `engine.io-client: ~6.6.1`). **Dropping the override alone is not sufficient and silently regresses `ws` to the vulnerable 8.20.1** — the lockfile still pins the old `engine.io`, so the removal has to be paired with `pnpm update -r --depth Infinity engine.io engine.io-client socket.io-adapter` in the same commit. This is the "consumers raised their declared floor" flavor of upstream catching up (like `picomatch` below), not the "patch published within the existing range" flavor — which is precisely why a bare `pnpm install` after deleting the block looks fine and isn't. The other three alerts in that batch (`esbuild` → 0.28.1, `@babel/core` → 7.29.7, `brace-expansion` → 5.0.6) needed **no** override — their fixed versions were already inside the consumers' declared ranges (`tsx` pins `esbuild: ~0.28.0`, react-hooks pins `@babel/core: ^7.24.4`, `minimatch` pins `brace-expansion: ^5.0.2`), so `pnpm update -r --depth Infinity <pkg>` pulled them. A prior batch of nine security overrides (`minimatch@3`/`minimatch@10`, `socket.io-parser`, `flatted`, `picomatch`, `yaml`, two `brace-expansion` majors, and `path-to-regexp`) was removed on 2026-06-10 once upstream caught up: every consumer's declared semver range now resolves to a CVE-fixed version on its own, so the overrides had become no-ops. Note that "upstream caught up" can mean two different things — most deps published a patch _within_ their existing range (e.g. `socket.io-parser@4.2.6` under `socket.io`'s `~4.2.4`), but `picomatch` was only freed because its _consumers_ (`tinyglobby`, `lint-staged`) raised their declared floor to `^4.0.4` (there is still no `picomatch > 4.0.4`). The `yaml` override was the subtle one: it forced `yaml@2.8.3` to satisfy Vite's _optional peer_ (`^2.4.2`), so its redundancy could only be confirmed empirically — removing it let Vite's peer resolve to `2.9.0` and deduped the lockfile's two `yaml` copies into one.
  ...
  - **Alerts are per package + manifest, not per resolved version.** A single alert stays open while _any_ in-range copy remains, so partial remediation won't close it. #36 was raised against `brace-expansion` with two copies in the tree — `5.0.7` (via `nodemon@3.1.14` → `minimatch@10.2.5`, fixed by `pnpm update -r --depth Infinity minimatch` since nodemon declares `^10.2.1` and `minimatch@10.2.6` → `brace-expansion@5.0.8` is in range) and `1.1.16` (via `minimatch@3.1.5`, pinned `^3.1.2` by `eslint-plugin-import` and `eslint-plugin-react`, both already latest). Only the first was fixable. **Outcome, re-checked 2026-09-09: `brace-expansion@1.1.16` is still in the tree** (`minimatch@3.1.5` is unchanged, still pinned `^3.1.2` by the two eslint plugins) — but the alert is gone, **auto-dismissed** by GitHub on 2026-08-05, not fixed. Two lessons follow. An alert disappearing is not evidence the dependency moved, so re-check `pnpm-lock.yaml` rather than reading the alert list as a scoreboard. And **alert numbers are not stable** — the `#36` cited above 404s today, because GitHub re-created the same advisory as #39/#40. The GHSA is the durable identifier; look alerts up by `.security_advisory.ghsa_id`, not by number. Always enumerate _every_ copy with `grep -n "^  <pkg>@" pnpm-lock.yaml` plus the `minimatch: <version>` consumer lines before estimating the work.
  ...
  - **A transitive-only alert never gets a Dependabot PR — fix it by hand.** The two alerts open on 2026-09-09 (`browserslist` GHSA-73wf-gq98-2v4g, `baseline-browser-mapping` GHSA-w5vr-8v7q-w6rv) reached the tree solely through `eslint-plugin-react-hooks → @babel/core → @babel/helper-compilation-targets → browserslist → baseline-browser-mapping`, with no manifest entry of their own — so Dependabot's pnpm updater could not open a PR, and never did. Nothing was waiting to be merged; the alerts would simply have sat there. Both fixed versions were already inside the consumers' declared ranges (`^4.24.0` and `^2.10.12`), i.e. the in-range flavor, so `pnpm update -r --depth Infinity browserslist baseline-browser-mapping` was the entire fix with no override. When the alert list is non-empty but the PR list is empty, assume a transitive dep and go update it yourself.
  - **`require`d is not the same as called — probe the invocation before rating urgency.** GHSA-73wf-gq98-2v4g is a High (7.5) crash in `browserslist`'s `normalizeStats()`, which the advisory notes runs on **every** `browserslist()` call regardless of query. `browserslist` genuinely _is_ loaded during `pnpm lint` (by `@babel/helper-compilation-targets`), so grepping for the entry point — the technique the `qs` bullet below prescribes — says "reachable." It isn't: with no `browserslist` config, no `.browserslistrc` and no `browserslist-stats.json` anywhere in the repo, Babel never resolves targets, so `browserslist()` is never invoked and the vulnerable function never runs. Settle this empirically instead of by reading code: run the real command under `NODE_OPTIONS="--require probe.cjs"` where `probe.cjs` patches `Module._load` to log the require and return a wrapper around the exported function that logs each call. Loaded-but-never-called is the difference between a High CVSS and a non-issue, and only the wrapper distinguishes them.
  - **A true `runtime` scope still isn't reachability — check the call path too.** #42 (`qs` array-limit bypass, GHSA-x5fp-wj9c-mxmx) was correctly labeled `runtime`: `pnpm why -r --prod qs` returns two real production paths, `express@5.2.1 → qs` and `express@5.2.1 → body-parser@2.3.0 → qs`. It was still unreachable, for reasons only the source shows. Express **5** defaults `query parser` to `'simple'` (Node's `querystring`, see `express/lib/application.js`) rather than Express 4's `'extended'`, so `qs` never parses a query string unless the app opts back in; `body-parser`'s `qs` use is confined to `urlencoded()`, and `index.ts` mounts only `express.json()`; and the bug additionally requires the non-default `comma: true`. So after `pnpm why` says "runtime", grep for the entry point (`query parser`, `urlencoded`, and the advisory's required options) before rating urgency — the graph says a package is present, not that it runs.
  - **`pnpm update` rewrites lockfile snapshot keys; `pnpm install` doesn't.** The #42 fix (`pnpm update -r --depth Infinity qs`) produced a 140/-122 lockfile diff for what is really three package changes (`qs` 6.15.2→6.16.0, `side-channel` 1.1.0→1.1.1, plus the new leaves `side-channel-list`/`es-define-property` that `qs@6.16.0` pulls). The other ~250 lines were `pnpm update` re-emitting snapshot keys **without** the `(supports-color@5.5.0)` optional-peer suffix — `express@5.2.1(supports-color@5.5.0):` → `express@5.2.1:` across express, socket.io, and the whole eslint subtree, which reads alarmingly like a resolution change and is not one. Confirm the churn is cosmetic the same way: the `supports-color@5.5.0` package entry and every `debug: 4.4.3(supports-color@5.5.0)` edge survive untouched, and a plain `pnpm install --lockfile-only` against the **pre-update** lockfile is a zero-line diff (so the rewrite comes from `update`, not from re-resolving). Read the diff with `git diff pnpm-lock.yaml | grep -E "^[-+]  [a-z@][^ ]*@"` to separate real version moves from key churn before assuming a bump touched more than it did.
