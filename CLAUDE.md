...
`ChampionsGraphic` (`apps/client/src/components/game/ChampionsGraphic.tsx`) renders the end-game "Spades Champions" image shown inline in `GameEndModal`. It draws a fixed template photo (`apps/client/src/assets/champions.png`, 1195×896) onto a `<canvas>` at native resolution (scaled down with `maxWidth: 100%`), then overlays four dynamic text fields — date, winner name, loser name, score (`winner > loser`) — at coordinates calibrated to the template's handwritten slots (the `FIELDS` constant).
