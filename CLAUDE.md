...
- **pnpm.overrides**: `minimatch@3` → `3.1.4` and `minimatch@10` → `10.2.3` to resolve Dependabot ReDoS alerts. Both are within the semver ranges their consumers (`eslint-plugin-import`, `eslint-plugin-react`, `eslint`, `nodemon`) already declare, so no compatibility risk. These can be removed once the upstream packages ship with patched minimatch versions. `socket.io-parser@>=4.0.0 <4.2.6` → `4.2.6` to fix CVE-2026-33151 (unbounded binary attachments DoS). Within the `~4.2.4` range declared by `socket.io@4.8.3` — can be removed once a newer `socket.io` release resolves 4.2.6 in its lockfile.
